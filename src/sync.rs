use heapless::Vec;
use core::fmt::Debug;
use crate::{Sh1107g, error::Sh1107gError, I2C_MAX_WRITE};
    ...
    E: Debug + embedded_hal::i2c::Error,
{
    fn send(&mut self, control:u8, data:&[u8])->Result<(),Sh1107gError<E>>{
        for chunk in data.chunks(I2C_MAX_WRITE-1){
            let mut payload=Vec::<u8,I2C_MAX_WRITE>::new();
    ...
    pub fn init(&mut self)->Result<(),Sh1107gError<E>>{
        ...
        for c in cmds{ self.send(0x80,c)?; }
        Ok(())
    }

    pub fn flush(&mut self)->Result<(),Sh1107gError<E>>{
        use crate::{DISPLAY_HEIGHT,DISPLAY_WIDTH,I2C_MAX_WRITE};
        let page_count=DISPLAY_HEIGHT/8;
        let page_width=DISPLAY_WIDTH;
        for page in 0..page_count{
            self.send_cmd(0xB0+page as u8)?;
            self.send_cmd(0x00)?; // 下位列
            self.send_cmd(0x10)?; // 上位列
            let start=page*page_width;
            let end=start+page_width;
            let mut page_buf=heapless::Vec::<u8,{I2C_MAX_WRITE-1}>::new();
            page_buf.extend_from_slice(&self.buffer[start..end]).map_err(|_|Sh1107gError::PayloadOverflow)?;
            self.send(0x40,&page_buf)?;
        }
        Ok(())
    }
}
